    ...
    <el-table :data="showData" @sort-change="sortChange">
      ...
      <el-table-column label="任课教师" show-overflow-tooltip>
        <template slot-scope="scope">
          <span
            v-for="(item,index) in scope.row.teacher"
            :key="item.u_id"
          >{{index==scope.row.teacher.length-1? item.u_name:item.u_name+', '}}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="create_time"
        ...
        :formatter="row=>this.$moment(row.create_time).format('YYYY-MM-DD')"
      ></el-table-column>
