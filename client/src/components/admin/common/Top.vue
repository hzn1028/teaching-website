  ...
  <div id="top" class="all-item-between">
    <div class="left all-item-between">
      ...
      <el-menu
        ...
        ref="topMenu"
        class="el-menu-demo"
        style="border-bottom:0px;margin-left:14px;"
      ...
      >
        <el-submenu v-if="courses.length>0" index="course">
          ...
          <el-menu-item
            v-for="(item,num) in courses"
            :index="'course:'+num"
            :key="item.c_id"
            :disabled="num==currentCourse"
    ...
    <div class="right">
      <el-dropdown @command="handleCommand" @visible-change="index=>isPersonInfoDropdown = index">
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item v-if="userInfo.u_type == 0" command="manage">管理</el-dropdown-item>
    ...
    updateChecked(path) {
      ...
      if (!path.startsWith("/admin/course/")) {
        ...
        if (path == "/admin")
          ...
          bus.$emit("leftOrder", {
            order: data[2],
            defaultIndex: data.slice(2).join("/")
          });
        }
        return;
      }
